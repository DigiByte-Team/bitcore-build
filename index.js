 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`digicore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`digicore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'digicore-' + name : 'digicore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/digicore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external digicore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:digicore-lib -o digicore-lib.js';
    }
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('digibyte-team', name, cb);
    });
  });
